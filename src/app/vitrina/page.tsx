          ...
          <div className="bg-white w-full max-w-2xl mx-auto rounded-t-3xl shadow-2xl h-[90dvh] flex flex-col relative animate-in slide-in-from-bottom-full duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="bg-pink-50 p-6 rounded-3xl space-y-4 border border-pink-100">
                ...
                <div>
                  <label className="block text-sm font-bold text-pink-900 mb-1">Tu número de WhatsApp *</label>
                  <input 
                    type="tel"
                    value={customerPhone}
                    onChange={(e) => setCustomerPhone(e.target.value)}
                    className="w-full bg-white border border-pink-200 rounded-2xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-pink-400"
                    placeholder="Ej. 8123456789"
                  />
                  <p className="text-xs text-muted-foreground mt-1 ml-1">A este número te contactaremos para confirmar tu cotización.</p>
                </div>

                <div>
                  <label className="block text-sm font-bold text-pink-900 mb-1">Fecha de entrega deseada *</label>
                  <input 
                    type="date"
                    value={deliveryDate}
                ...
                <div>
                  <label className="block text-sm font-bold text-pink-900 mb-1">Detalles o comentarios (Opcional)</label>
                  <textarea 
                    value={details}
                    onChange={(e) => setDetails(e.target.value)}
                    className="w-full bg-white border border-pink-200 rounded-2xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-pink-400 resize-none h-24"
                    placeholder="Temática, colores, etc."
                  />
                </div>
              </div>
            </div>

            <div className="p-4 bg-white border-t border-pink-100 sticky bottom-0 z-10 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]">
              <Button 
                onClick={submitQuote} 
                disabled={isSubmitting}
                className="w-full rounded-2xl h-14 text-lg font-bold bg-[#25D366] hover:bg-[#1ebd5a] text-white shadow-lg shadow-green-500/25 flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
